...
import { featuredStay } from '@/data/hotels'

const SUBTITLE =
  'Slow travel, unforgettable stays. Our editors pick the places we would book ourselves — with free cancellation on most of them.'

/** Зелёная карточка «Featured stay» + сетка фото 2×2. */
export default function FeaturedStay() {
  const { title, price, text, images } = featuredStay

  return (
    <section className="container-page flex flex-col gap-6">
      <SectionHeader title="Unique places to stay" subtitle={SUBTITLE} actionTo="/hotels/list" />
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        <Reveal variant="left" className="lg:w-[552px] lg:shrink-0">
          ...
          >
            ...
            <Button to="/hotels/list" variant="white" className="w-full">
              Book Hotel
            </Button>
          </Tilt>
        </Reveal>
